...
class ZoomGPUImageView : GLImageView {
    private var zoomEnable = true
    private var scrollEnable = true
    fun setZoomEnable(zoomEnable: Boolean) {
    ...
    fun getInnerMatrix(matrix: Matrix?): Matrix {
        ...
        if (isReady) {
            //原图大小
            val tempSrc: RectF = MathUtils.rectFTake(0f, 0f, getImageWidth().toFloat(), getImageHeight().toFloat())
            //控件大小
    ...
    fun getImageBound(rectF: RectF?): RectF {
        ...
        return if (!isReady) {
            ...
            //对原图矩形进行变换得到当前显示矩形
            rectF[0f, 0f, getImageWidth().toFloat()] = getImageHeight().toFloat()
    ...
    private var doubleTapDisabled = false
    fun setDoubleTapDisabled(doubleTapDisabled: Boolean) {
        this.doubleTapDisabled = doubleTapDisabled
    }
    ////////////////////////////////公共状态设置////////////////////////////////
    /**
    ...
    fun outerMatrixTo(endMatrix: Matrix?, duration: Long) {
        ...
        if (duration <= 0) {
            //计算变化矩阵
            val outInvert = Matrix()
            mOuterMatrix.invert(outInvert)
            outInvert.postConcat(endMatrix)
            val outerDiffMatrix = Matrix(outInvert)
            mOuterMatrix.set(endMatrix)
            dispatchOuterMatrixChanged(outerDiffMatrix)
            updateMatrix()
        } else {
            //创建矩阵变化动画
    ...
    interface OuterMatrixChangedListener {
         ...
         */
        fun onOuterMatrixChanged(transformedMatrix: Matrix?)
    }
     ...
     * @see .mOuterMatrix
     */
    private fun dispatchOuterMatrixChanged(diffMatrix: Matrix) {
        if (mOuterMatrixChangedListeners == null) {
            return
        }
        //增加锁
        ...
        for (listener in mOuterMatrixChangedListeners!!) {
            listener.onOuterMatrixChanged(diffMatrix)
        }
        //减锁
        mDispatchOuterMatrixChangedLock--
     ...
     * @see .getMaxScale
     */
    protected fun calculateNextScale(outerScale: Float): Float {
        return if (outerScale < DOUBLE_TAB_MAX_SCALE) DOUBLE_TAB_MAX_SCALE else 1f
    }
    ...
    private val isReady: Boolean
        private get() = getImageWidth() > 0 && getImageHeight() > 0 && getWidth() > 0 && getHeight() > 0
    ////////////////////////////////mask动画处理////////////////////////////////
    /**
     * mask修改的动画
     *
     ...
     * 在onTouchEvent末尾被执行.
     */
    private val mGestureDetector = GestureDetector(this@ZoomGPUImageView.getContext(), object : SimpleOnGestureListener() {
        override fun onFling(e1: MotionEvent, e2: MotionEvent, velocityX: Float, velocityY: Float): Boolean {
            //只有在单指模式结束之后才允许执行fling
            if (pinchMode == PINCH_MODE_FREE && !(mScaleAnimator != null && mScaleAnimator!!.isRunning) && scrollEnable) {
        ...
        override fun onLongPress(e: MotionEvent) {
            ...
            if (mOnLongClickListener != null) {
                mOnLongClickListener?.onLongClick(this@ZoomGPUImageView)
            }
        }

        override fun onDoubleTap(e: MotionEvent): Boolean {
            //当手指快速第二次按下触发,此时必须是单指模式才允许执行doubleTap
            if (!doubleTapDisabled && pinchMode == PINCH_MODE_SCROLL && !(mScaleAnimator != null && mScaleAnimator!!.isRunning)) {
                doubleTap(e.x, e.y)
            }
            return true
        }
        ...
        override fun onSingleTapConfirmed(e: MotionEvent): Boolean {
            //触发点击
            if (!doubleTapDisabled && mOnClickListener != null) {
                mOnClickListener?.onClick(this@ZoomGPUImageView)
            }
            return true
        }
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        if (action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_CANCEL) {
            ...
            if (!(mScaleAnimator != null && mScaleAnimator!!.isRunning)) {
                //在滚动模式下移动
                if (pinchMode == PINCH_MODE_SCROLL && !isMoveLock && scrollEnable) {
                    //每次移动产生一个差值累积到图片位置上
                    ...
                    //两个缩放点间的距离
                    val distance: Float = MathUtils.getDistance(event.getX(0), event.getY(0), event.getX(1), event.getY(1))
                    //保存缩放点中点
                    val lineCenter: FloatArray = MathUtils.getCenterPoint(event.getX(0), event.getY(0), event.getX(1), event.getY(1))
    ...
    private fun scrollBy(xDiff: Float, yDiff: Float): Boolean {
        ...
        //计算过渡矩阵
        val outerDiffMatrix = Matrix()
        outerDiffMatrix.postTranslate(xDiff, yDiff)
    ...
    private fun saveScaleContext(x1: Float, y1: Float, x2: Float, y2: Float) {
        ...
        mScaleBase = MathUtils.getMatrixScale(mOuterMatrix).get(0) / MathUtils.getDistance(x1, y1, x2, y2)
        ...
        val center: FloatArray = MathUtils.inverseMatrixPoint(MathUtils.getCenterPoint(x1, y1, x2, y2), mOuterMatrix)
    ...
    private fun doubleTap(x: Float, y: Float) {
        ...
        //接下来要放大的大小
        var nextScale = if (calculateNextScale(outerScale) == 1f) innerScale else calculateNextScale(outerScale)
        ...
        val testBound: RectF = MathUtils.rectFTake(0f, 0f, getImageWidth().toFloat(), getImageHeight().toFloat())
    ...
    private var isMoveLock = false
    fun setMoveLock(isMoveLock: Boolean) {
        this.isMoveLock = isMoveLock
    }
    ...
    companion object {
        ...
        private const val DOUBLE_TAB_MAX_SCALE = 2f
        ////////////////////////////////公共状态获取////////////////////////////////
        /**
         * 手势状态：自由状态
         *
    ...
    object MathUtils {
        ...
        fun matrixTake(): Matrix {
            return mMatrixPool.take() ?: Matrix()
        }
        ...
        fun matrixTake(matrix: Matrix?): Matrix {
            val result: Matrix = mMatrixPool.take() ?: Matrix()
            if (matrix != null) {
                result.set(matrix)
            }
            return result
        ...
        fun rectFTake(left: Float, top: Float, right: Float, bottom: Float): RectF {
            val result: RectF = mRectFPool.take() ?: RectF()
    ...
    private class MatrixPool(size: Int) : ObjectsPool<Matrix?>(size) {
        protected override fun newInstance(): Matrix {
            return Matrix()
        }
    ...
    private class RectFPool(size: Int) : ObjectsPool<RectF?>(size) {
        protected override fun newInstance(): RectF {
            return RectF()
        }
    ...
    abstract class ObjectsPool<T>(val mSize: Int = 0) {


        /**
         * 对象池队列
         */
        private lateinit var mQueue: Queue<T>
